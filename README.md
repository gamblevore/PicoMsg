...
Passing a non-zero number to ExecSocket means that you are preparing for a call to any of the `execve()`. Its kinda complex to explain, so just look at PicoTest.cpp for a good example. The main thing this does is let you choose a FD number, and delays reading of messages.
